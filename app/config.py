...
class Settings(BaseSettings):
    ...
    # LOCAL_TOOLS is now 20 (read_document, read_image, edit_excel,
    # nepali_date and read_department_doc all landed after that measurement). Serialising those same 15
    # schemas today gives 15,498 chars, so the measurement calibrates to ~4.46
    # chars/token; all 20 serialise to ~22,820 chars, i.e. **~5120 tokens
    # estimated**. Raised 4000 -> 4600 -> 4900 -> 5200 on that basis
    # (2026-08-29): the
    # value was ALREADY below the estimate at 17 tools, and under-reserving is
    # the dangerous direction — we budget history confidently into an overflow
    # and Ollama drops the FRONT of the prompt, where the identity and date
    # system prompt lives. This is an ESTIMATE, not a measurement: re-measure
    # with usage.prompt_tokens against the deployed model (qwen3.5:35b-a3b
    # tokenizes differently from qwen2.5). At 20 tools this is ~16% of the
    # window spent before anyone types — the case for gating tool exposure on
    # what the turn actually has (attachments, a bound department) grows with
    # every addition.
    context_tool_schema_tokens: int = 5200
